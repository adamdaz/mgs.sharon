...
```{r}
tic()
mgs.rd.data.site1a <- read.table(file = "mgs.rd.site1.txt", header = TRUE, colClasses = c("character", "factor", "factor", "factor", "factor"), col.names = c("ch", "captage", "sex", "id", "site") )
mgs.rd.data.site2a <- read.table(file = "mgs.rd.site2.txt", header = TRUE, colClasses = c("character", "factor", "factor", "factor", "factor"), col.names = c("ch", "captage", "sex", "id", "site") )
mgs.rd.data.site3a <- read.table(file = "mgs.rd.site3.txt", header = TRUE, colClasses = c("character", "factor", "factor", "factor", "factor"), col.names = c("ch", "captage", "sex", "id", "site") )
mgs.rd.data.site4a <- read.table(file = "mgs.rd.site4.txt", header = TRUE, colClasses = c("character", "factor", "factor", "factor", "factor"), col.names = c("ch", "captage", "sex", "id", "site") )
...
mgs.rd.allc$captage1 <- as.factor(ifelse(mgs.rd.allc$captage=="ADU",1,0))
...
```{r}
tic()
mgs.rd.pd <- process.data(data = mgs.rd.all, model = "Robust", groups = c("sex", "site", "captage1"), time.intervals = mgs.time.intervals.rd.yr, age.var = 3, initial.ages = c(1,0), age.unit = 1)
...
site1.fixed.pc <- read.csv("site1.fixed.pc.csv", header = TRUE, fill = TRUE)
site2.fixed.pc <- read.csv("site2.fixed.pc.csv", header = TRUE, fill = TRUE)
site3.fixed.pc <- read.csv("site3.fixed.pc.csv", header = TRUE, fill = TRUE)
site4.fixed.pc <- read.csv("site4.fixed.pc.csv", header = TRUE, fill = TRUE)

#input the day from start for each element in the ch
days.from.start <-(read.csv("days.from.start.csv", header = TRUE))
